...
/**
 * Labels tab embedded in Project Detail.
 * TODO_BACKEND: Filter labels by projectId when API supports it.
 * Currently fetches all labels (no project filter on BE).
 */
export default function ProjectLabels() {
    ...
    const loadLabels = async () => {
        setLoading(true);
        try {
            // TODO_BACKEND: use getLabelsByProject(projectId) when available
    ...
    const filtered = labels.filter((l: any) =>
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h2 className="text-base font-bold text-foreground">Labels</h2>
            ...
            <Card className="p-6 space-y-4">
                <Input placeholder="Tìm kiếm nhãn..." value={search} onChange={(e) => setSearch(e.target.value)} className="max-w-sm" />

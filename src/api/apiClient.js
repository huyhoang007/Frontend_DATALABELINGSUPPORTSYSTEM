    ...
    (config) => {
        ...
        if (config.baseURL && config.baseURL.replace(/\/$/, '').endsWith('/api') && config.url?.startsWith('/api/')) {
            ...
            if (import.meta.env.DEV) {
                console.warn('[API] Double /api prefix detected — auto-stripped. Fix VITE_API_BASE_URL (should be "/" not "/api").');
            }
        }
...
apiClient.interceptors.response.use(
    ...
    (error) => {
        ...
        if (error.response && error.response.status === 403) {
            ...
            console.warn("[API] 403 Forbidden - Bạn không có quyền thực hiện hành động này");
